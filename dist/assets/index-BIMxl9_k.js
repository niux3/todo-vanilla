...
`);a+=`return output;`;let s=`with(data) { ${a} }`;o(this,h).wrappers&&o(this,h).wrappers.forEach(e=>{s=e(s,a)});try{let e=[`data`,`escapeHTML`,...o(this,h).extraParams||[]],n=Function(...e,s);return o(this,l).size>=o(this,u)&&o(this,l).delete(o(this,l).keys().next().value),o(this,l).set(t,n),n}catch(e){throw Error(`Template compilation failed: ${e.message}`)}};var y=class extends HTMLElement{static get observedAttributes(){return[`data`]}constructor(){super(),this.engine=new v,this.li=null,this._todos=[]}attributeChangedCallback(e,t,n){if(e===`data`&&n)try{this._todos=JSON.parse(n),this.render()}catch(e){console.error(`Invalid JSON in data attribute`,e)}}connectedCallback(){this.render()}render(){let e={todos:this._todos};this.innerHTML=this.engine.render(r,e),this.li=this.querySelectorAll(`li`),this.li.forEach(e=>{e.querySelector(`button`).addEventListener(`click`,this.handleRemove.bind(this)),e.addEventListener(`change`,this.handleToggle.bind(this))})}disconnectedCallback(){this.li.forEach(e=>{e.querySelector(`button`).removeEventListener(`click`,this.handleRemove.bind(this)),e.querySelector(`input[type="checkbox"]`).removeEventListener(`click`,this.handleToggle.bind(this))})}handleRemove(e){let t=e.target;if(t){e.preventDefault();let n=t.closest(`li`);if(n){let e=n.dataset.id;this.dispatchEvent(new CustomEvent(`remove`,{detail:{id:e},bubbles:!0,composed:!0}))}}}handleToggle(e){if(e.target){let t=e.target.closest(`li`).dataset.id;this.dispatchEvent(new CustomEvent(`toggle`,{detail:{id:t},bubbles:!0}))}}set data(e){this._todos=e||[],this.render()}},b=`<footer>
    <span>[[= todos.length ]] tâche[[= todos.length > 1 ? 's' : '']]</span>
    <nav role="navigation">
        <a href="#/all" class="button">toutes</a>
        <a href="#/pending" class="button">en cours</a>
        <a href="#/completed" class="button">terminées</a>
        <a href="#/clear" class="button">effacer</a>
    </nav>
</footer>
`,x=class extends HTMLElement{static get observedAttributes(){return[`data`]}constructor(){super(),this.engine=new v,this._todos=[]}attributeChangedCallback(e,t,n){if(e===`data`&&n)try{this._todos=JSON.parse(n),this.render()}catch(e){console.error(`Invalid JSON in data attribute`,e)}}connectedCallback(){this.render()}render(){let e=this.engine.render(b,{todos:this._todos});this.innerHTML=e}},S=class extends HTMLElement{constructor(){super(),customElements.define(`todo-form`,n),customElements.define(`todo-list`,y),customElements.define(`todo-footer`,x),this.todoManager=new e}connectedCallback(){this.todoManager.load(),this.insertAdjacentHTML(`beforeend`,`<todo-form />`),this.todoForm=this.querySelector(`todo-form`),this.todoForm.addEventListener(`add`,this.handleAdd.bind(this)),this.todoForm.addEventListener(`toggleAll`,this.handleToggleAll.bind(this)),this.insertAdjacentHTML(`beforeend`,`<todo-list />`),this.todoList=this.querySelector(`todo-list`),this.todoList.addEventListener(`remove`,this.handleRemove.bind(this)),this.todoList.addEventListener(`toggle`,e=>this.handleToggle(e)),this.insertAdjacentHTML(`beforeend`,`<todo-footer />`),this.todoFooter=this.querySelector(`todo-footer`),this.handleRoute()}disconnectedCallback(){window.removeEventListener(`load`,this._onLoad),window.removeEventListener(`hashchange`,this._onHashChange)}handleRoute(){this._onLoad=()=>this.updateListWithCurrentFilter(),this._onHashChange=()=>this.updateListWithCurrentFilter(),window.addEventListener(`load`,this._onLoad),window.addEventListener(`hashchange`,this._onHashChange),this.updateListWithCurrentFilter()}handleToggleAll(e){this.todoManager.toggleAll(e.detail.checked),this.todoManager.save(),this.updateListWithCurrentFilter()}handleAdd(e){this.todoManager.add(e.detail.title),this.todoManager.save(),this.updateListWithCurrentFilter()}handleRemove(e){let t=e.detail.id;this.todoManager.remove(t),this.todoManager.save(),this.updateListWithCurrentFilter()}handleToggle(e){let t=e.detail.id;this.todoManager.toggle(t),this.todoManager.save(),this.updateListWithCurrentFilter()}updateListWithCurrentFilter(){let e=window.location.hash||`#/all`,t;switch(e){case`#/all`:t=this.todoManager.getAll();break;case`#/pending`:t=this.todoManager.getPending();break;case`#/completed`:t=this.todoManager.getCompleted();break;case`#/clear`:this.todoManager.clear(),this.todoManager.save(),t=this.todoManager.getAll();break;default:t=this.todoManager.getAll()}this.updateList(t)}updateList(e){this.todoList.setAttribute(`data`,JSON.stringify(e)),this.todoFooter.setAttribute(`data`,JSON.stringify(e))}};customElements.define(`todo-component`,S);